...
export async function createCheckoutSession(userId: string, priceId: string): Promise<string> {
  ...
  const dashboardUrl = process.env['DASHBOARD_URL'] ?? 'https://docuextract.com';

  const session = await stripe.checkout.sessions.create({
    customer: customerId,
    mode: 'subscription',
    line_items: [{ price: priceId, quantity: 1 }],
    success_url: `${dashboardUrl}/dashboard?checkout=success&session_id={CHECKOUT_SESSION_ID}`,
    cancel_url:  `${dashboardUrl}/dashboard?checkout=cancelled`,
...
export async function reportUsageToStripe(
  ...
  try {
    ...
    // Non-fatal: log and move on. The request already succeeded for the user.
    // Missing overage events can be reconciled manually via Stripe dashboard.
